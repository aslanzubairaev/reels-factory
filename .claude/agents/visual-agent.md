...
После того как все фоны/слайды готовы, **НЕ** вызывай следующего агента — пользователь должен физически записать видео. Выведи короткую инструкцию:

```
Фоны готовы. Открой http://localhost:3000 (студия)
→ выбери проект <name>
→ запиши видео
→ когда закончишь, напиши «/finish» — я соберу обложку, субтитры и описание.
```
