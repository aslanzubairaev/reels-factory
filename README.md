...
> Для `--with-subs` нужен дополнительный пакет: `pip install openai-whisper` (тянет torch, ~2 GB; модель `medium` скачивается при первом запуске). В основной `requirements.txt` он не включён, чтобы не нагружать тех, кто субтитры не использует.
